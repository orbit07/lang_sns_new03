...
function renderTodayReviewCard(card) {
  ...
  const syncFaceState = () => {
    const showFront = vocabularyReviewState.showFront;
    cardEl.classList.toggle('is-flipped', !showFront);
    frontFace?.setAttribute('aria-hidden', showFront ? 'false' : 'true');
    backFace?.setAttribute('aria-hidden', showFront ? 'true' : 'false');

    const text = showFront ? '裏を見る' : '表に戻る';
